        ...
        for ($attempt = 1; $attempt <= self::MAX_ATTEMPTS; $attempt++) {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::warning(
                    'Analyse KYC OpenAI : tentative échouée',
                    [
                        'client_id' => $client->id,
                        'attempt' => $attempt,
                        'error' => $e->getMessage(),
                ...
                // On renvoie la réponse invalide à l'IA avec un message
                // correctif avant de retenter, plutôt que d'abandonner
                // dès le premier échec.
                $messages[] = [
                    'role' => 'assistant',
                    'content' => $raw ?? 'Aucune réponse exploitable.',
                ];
                $messages[] = [
                    'role' => 'user',
                    'content' => "Ta réponse précédente est invalide : {$e->getMessage()}. Corrige-la et renvoie uniquement le JSON attendu, en respectant strictement le format demandé.",
                ];
            }
        }
