...
class PatrimoineController extends Controller
{
    ...
    ): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'elements.*.montant' => ['nullable', 'numeric', 'min:0'],
            'elements.*.mode_detention' => ['nullable', 'string', 'max:255'],
            'elements.*.type_pret' => ['nullable', 'string', 'max:255'],
            'elements.*.date_souscription' => ['nullable', 'date', 'before_or_equal:today'],
            'elements.*.duree' => ['nullable', 'integer', 'min:0', 'max:30'],
            'elements.*.taux_interet' => ['nullable', 'numeric', 'min:0'],
            'elements.*.taux_assurance' => ['nullable', 'numeric', 'min:0'],
            ...
            'elements.*.periodicite' => ['nullable', 'string', 'in:annuel,mensuel'],
        ...
        foreach ($validated['elements'] ?? [] as $element) {
            ...
            $client->patrimoineElements()->create([
                ...
                'periodicite' => $element['periodicite'] ?? 'annuel',
            ]);
        }
