...
<div class="page">
    ...
    <div class="titre">
        ...
        <h1>Mandat de courtage en assurance véhicule</h1>
    </div>
    ...
    <p class="corps-paragraphe">Le présent mandat confie au Courtier la recherche, la négociation et le suivi d'un contrat d'assurance automobile adapté au véhicule et à l'usage du Mandant. Le Courtier s'engage à analyser les besoins du Mandant et à lui proposer des garanties adaptées parmi les offres des compagnies d'assurance partenaires.</p>
    ...
    <ul class="corps-liste">
        ...
        <li>Réaliser une analyse objective des besoins avant toute recommandation, en tenant compte du véhicule, de son usage et des antécédents du conducteur.</li>
        ...
        <li>Fournir des explications claires sur les garanties, franchises et exclusions applicables.</li>
        <li>Assister le Mandant dans la souscription et, le cas échéant, la résiliation du contrat précédent (loi Hamon).</li>
    </ul>

    <div class="section-title">5. Obligations du Mandant</div>
    <p class="corps-paragraphe">Le Mandant s'engage à fournir des informations exactes et complètes sur le véhicule, son usage, les conducteurs et leurs antécédents, et à informer sans délai le Courtier de toute modification pouvant impacter le contrat.</p>
    ...
    <div class="recueil-sous-titre">Véhicule à assurer</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Immatriculation</td><td>{{ $recueil['immatriculation'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Marque / Modèle</td><td>{{ $recueil['marque'] ?? '-' }} {{ $recueil['modele'] ?? '' }}</td></tr>
        <tr><td class="recueil-label">Date de première immatriculation</td><td>{{ $recueil['date_premiere_immatriculation'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Genre</td><td>{{ $recueil['genre_vehicule_libelle'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Énergie</td><td>{{ $recueil['energie_libelle'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Valeur du véhicule</td><td>{{ $recueil['valeur_vehicule'] ?? '-' }} €</td></tr>
        <tr><td class="recueil-label">Kilométrage actuel</td><td>{{ $recueil['kilometrage_actuel'] ?? '-' }} km</td></tr>
    </table>

    <div class="recueil-sous-titre">Usage et stationnement</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Usage du véhicule</td><td>{{ $recueil['usage_vehicule_libelle'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Stationnement habituel</td><td>{{ $recueil['stationnement_libelle'] ?? '-' }}</td></tr>
    </table>

    @if(!empty($recueil['conducteurs']))
    <div class="recueil-sous-titre">Conducteurs</div>
    <table class="recueil-table">
        @foreach($recueil['conducteurs'] as $conducteur)
        <tr><td class="recueil-label">{{ $conducteur['nom'] ?? '-' }} ({{ $conducteur['statut_libelle'] ?? '-' }})</td><td>Né(e) le {{ $conducteur['date_naissance'] ?? '-' }} — Permis obtenu le {{ $conducteur['date_permis'] ?? '-' }}</td></tr>
        @endforeach
    </table>
    @endif

    <div class="recueil-sous-titre">Historique du conducteur principal</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Coefficient bonus-malus</td><td>{{ $recueil['bonus_malus'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Sinistres responsables (3 dernières années)</td><td>{{ $recueil['nombre_sinistres_responsables_3ans'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Résiliation antérieure</td><td>{{ $recueil['resiliation_anterieure_libelle'] ?? '-' }}</td></tr>
        @if(!empty($recueil['resiliation_anterieure_precision']))
        <tr><td class="recueil-label">Précision</td><td>{{ $recueil['resiliation_anterieure_precision'] }}</td></tr>
        @endif
    </table>

    <div class="recueil-sous-titre">Garanties et franchise</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Garanties souhaitées</td><td>{{ $recueil['garanties_souhaitees_libelle'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Franchise souhaitée</td><td>{{ $recueil['franchise_souhaitee_libelle'] ?? '-' }}</td></tr>
    </table>
