...
<div class="page">
    ...
    <div class="titre">
        ...
        <h1>Mandat de courtage en plan épargne retraite</h1>
    </div>
    ...
    <p class="corps-paragraphe">Le présent mandat confie au Courtier la recherche, la négociation et le suivi d'un plan d'épargne retraite individuel (PER) adapté à la situation professionnelle, fiscale et patrimoniale du Mandant, ainsi que, le cas échéant, le transfert de contrats d'épargne retraite existants. Le Courtier s'engage à analyser les besoins du Mandant et à lui proposer des solutions adaptées parmi les offres des compagnies d'assurance et gestionnaires partenaires.</p>
    ...
    <ul class="corps-liste">
        <li>Commissions versées par les assureurs et gestionnaires dans le cadre des contrats souscrits par le Mandant.</li>
    ...
    <ul class="corps-liste">
        ...
        <li>Réaliser une analyse objective des besoins avant toute recommandation, en tenant compte de la situation fiscale et professionnelle du Mandant.</li>
        ...
        <li>Fournir des explications claires sur le fonctionnement du PER, les options fiscales à l'entrée et à la sortie, les cas de déblocage anticipé prévus par la loi et les frais applicables.</li>
        <li>Assister le Mandant dans la souscription, le suivi et, le cas échéant, le transfert de contrats existants.</li>
    </ul>

    <div class="section-title">5. Obligations du Mandant</div>
    <p class="corps-paragraphe">Le Mandant s'engage à fournir des informations exactes et complètes nécessaires à l'étude, notamment sur sa situation professionnelle et fiscale, et à informer sans délai le Courtier de toute modification pouvant impacter le contrat.</p>
    ...
    <div class="recueil-sous-titre">Situation professionnelle et fiscale</div>
    <table class="recueil-table">
        ...
        <tr><td class="recueil-label">Option fiscale à l'entrée</td><td>{{ $recueil['deduction_fiscale_libelle'] ?? '-' }}</td></tr>
    </table>
    ...
    <div class="recueil-sous-titre">Horizon et versements</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Âge actuel</td><td>{{ $recueil['age_actuel'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Âge de départ à la retraite envisagé</td><td>{{ $recueil['age_retraite_envisage'] ?? '-' }}</td></tr>
    ...
    @if(!empty($recueil['transferts_contrats']))
    <div class="recueil-sous-titre">Transferts de contrats existants</div>
    <table class="recueil-table">
        @foreach($recueil['transferts_contrats'] as $transfert)
        <tr><td class="recueil-label">{{ $transfert['type_contrat_libelle'] ?? '-' }} — {{ $transfert['organisme'] ?? '-' }}</td><td>Valeur à transférer : {{ $transfert['valeur_transferee'] ?? '-' }} €</td></tr>
        @endforeach
    </table>
    @endif

    <div class="recueil-sous-titre">Gestion</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Mode de gestion souhaité</td><td>{{ $recueil['mode_gestion_libelle'] ?? '-' }}</td></tr>
        <tr><td class="recueil-label">Profil d'allocation</td><td>{{ $recueil['profil_allocation_libelle'] ?? '-' }}</td></tr>
    </table>

    <div class="recueil-sous-titre">Modalités de sortie souhaitées</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Sortie envisagée</td><td>{{ $recueil['modalite_sortie_libelle'] ?? '-' }}</td></tr>
    </table>

    @if(!empty($recueil['beneficiaires_deces']))
    <div class="recueil-sous-titre">Bénéficiaires en cas de décès avant liquidation</div>
    <table class="recueil-table">
        @foreach($recueil['beneficiaires_deces'] as $beneficiaire)
