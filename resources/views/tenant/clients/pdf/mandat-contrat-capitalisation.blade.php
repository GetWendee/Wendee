...
<div class="page">
    ...
    <div class="titre">
        ...
        <h1>Mandat de courtage en contrat de capitalisation</h1>
    </div>
    ...
    <p class="corps-paragraphe">Le présent mandat confie au Courtier la recherche, la négociation et le suivi d'un ou plusieurs contrats de capitalisation adaptés aux objectifs d'épargne, de valorisation de trésorerie ou de transmission du Mandant. Le Courtier s'engage à analyser les besoins du Mandant et à lui proposer des contrats adaptés parmi les offres des compagnies d'assurance partenaires. Il est rappelé que le contrat de capitalisation, à la différence de l'assurance vie, ne comporte pas de clause bénéficiaire hors succession et entre, sauf démembrement de propriété organisé, dans l'actif successoral du souscripteur.</p>
    ...
    <ul class="corps-liste">
        ...
        <li>Fournir des explications claires sur le régime fiscal et successoral du contrat de capitalisation, distinct de celui de l'assurance vie.</li>
        <li>Assister le Mandant dans la souscription, le suivi et, le cas échéant, la mise en place d'un démembrement de propriété.</li>
    </ul>
    ...
    <div class="recueil-sous-titre">Souscripteur</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Type de souscripteur</td><td>{{ $recueil['type_souscripteur_libelle'] ?? '-' }}</td></tr>
        @if(!empty($recueil['raison_sociale_souscripteur']))
        <tr><td class="recueil-label">Raison sociale</td><td>{{ $recueil['raison_sociale_souscripteur'] }}</td></tr>
        <tr><td class="recueil-label">SIREN</td><td>{{ $recueil['siren_souscripteur'] ?? '-' }}</td></tr>
        @endif
    </table>
    ...
    <div class="recueil-sous-titre">Mode de détention</div>
    <table class="recueil-table">
        <tr><td class="recueil-label">Détention souhaitée</td><td>{{ $recueil['mode_detention_libelle'] ?? '-' }}</td></tr>
    </table>
    @if(!empty($recueil['beneficiaires_demembrement']))
    <table class="recueil-table">
        @foreach($recueil['beneficiaires_demembrement'] as $beneficiaire)
        <tr><td class="recueil-label">{{ $beneficiaire['nom'] ?? '-' }} ({{ $beneficiaire['lien'] ?? '-' }})</td><td>Nue-propriété : {{ $beneficiaire['quote_part_nue_propriete'] ?? '-' }}</td></tr>
        @endforeach
    </table>
    @endif
