...
@php
    ...
    $besoinsSelectionnes = $donnees['besoins_specifiques'] ?? [];
...
.wd-mandat-garantie-row{display:grid;grid-template-columns:1fr 200px;gap:12px;align-items:center;margin-bottom:10px;}
.wd-mandat-garantie-row label{font-size:12px;font-weight:700;color:var(--ink);}
...
<section class="wd-section">
    <div class="wd-mandat-card">
        <h2>Mandat de courtage en complémentaire santé</h2>
    ...
    <div class="wd-mandat-body">
        <div class="wd-mandat-header-status">
            ...
            @if($mandat)
                <button type="button" id="wd-mandat-pdf-btn" class="wd-mandat-pdf-btn" data-pdf-url="{{ route('tenant.clients.mandat-complementaire-sante.pdf', $client) }}" data-lieu-defaut="{{ $client->kyc?->lieu_signature ?: $cabinet?->ville }}">
                    Télécharger le mandat en PDF
                </button>
            @endif
        </div>

        <form method="POST" action="{{ route('tenant.clients.mandat-complementaire-sante.enregistrer', $client) }}">
            @csrf

            <div class="wd-mandat-section-title">1. Situation actuelle</div>
            <div class="wd-mandat-field">
                <div class="wd-cabinet-radio-group">
                    @foreach($regimeOptions as $valeur => $label)
                        <label class="wd-cabinet-radio">
                            <input type="radio" name="regime_obligatoire" value="{{ $valeur }}" {{ ($donnees['regime_obligatoire'] ?? '') === $valeur ? 'checked' : '' }} {{ $valeur === 'autre' ? 'data-toggle-target="champ-regime-autre"' : '' }}>
                            <span>{{ $label }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
            <div class="wd-mandat-field" id="champ-regime-autre" style="display:{{ ($donnees['regime_obligatoire'] ?? '') === 'autre' ? '' : 'none' }};">
                <label class="wd-mandat-label">Précision si "Autre"</label>
                <input type="text" name="regime_obligatoire_autre_precision" class="wd-mandat-input" value="{{ $donnees['regime_obligatoire_autre_precision'] ?? '' }}">
            </div>
            <div class="wd-mandat-row wd-mandat-row-2">
                <div class="wd-mandat-field">
                    <label class="wd-mandat-label">Mutuelle actuelle</label>
            ...
            <div class="wd-mandat-field">
                <label class="wd-mandat-label">Motif du changement</label>
                <input type="text" name="motif_changement" class="wd-mandat-input" value="{{ $donnees['motif_changement'] ?? '' }}">
            </div>

            <div class="wd-mandat-section-title">2. Personnes à couvrir</div>
            ...
            <div class="wd-mandat-section-title" style="margin-top:24px;">3. Niveaux de garanties souhaités par poste</div>
            @foreach($postesGarantie as $poste => $labelPoste)
                <div class="wd-mandat-garantie-row">
                    <label>{{ $labelPoste }}</label>
                    <select name="niveaux_garanties[{{ $poste }}]" class="wd-mandat-select">
                        <option value="">Non prioritaire</option>
                        @foreach($niveauOptions as $valeur => $label)
                            <option value="{{ $valeur }}" {{ ($donnees['niveaux_garanties'][$poste] ?? '') === $valeur ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            @endforeach

            <div class="wd-mandat-section-title">4. Besoins spécifiques</div>
            <div class="wd-mandat-field">
                <div class="wd-cabinet-checkbox-group">
                    @foreach($besoinsOptions as $valeur => $label)
                        <label class="wd-cabinet-checkbox">
                            <input type="checkbox" name="besoins_specifiques[]" value="{{ $valeur }}" {{ in_array($valeur, $besoinsSelectionnes) ? 'checked' : '' }}>
            ...
            <div class="wd-mandat-section-title">5. Budget</div>
            <div class="wd-mandat-field">
                <label class="wd-mandat-label">Budget mensuel souhaité (€)</label>
                <input type="number" step="0.01" name="budget_mensuel_souhaite" class="wd-mandat-input" value="{{ $donnees['budget_mensuel_souhaite'] ?? '' }}">
            </div>
...
<div id="wd-modal-lieu" class="wd-modal-overlay" style="display:none;">
    <div class="wd-modal-card">
        ...
        <div class="wd-modal-eyebrow">Mandat complémentaire santé</div>
