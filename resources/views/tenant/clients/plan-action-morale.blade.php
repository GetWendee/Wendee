    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Plan d'action - {{ $client->nomAffichage() }}
            </h2>
...
<div class="wd-reco-card">
    ...
    <div>
        ...
        <p>Basé sur le KYC, le patrimoine, le profil investisseur et la recommandation patrimoniale société</p>
    </div>
</div>
<div class="wd-reco-body">
    ...
    @endif
    <p class="wd-reco-date">Date du plan d'action : {{ now()->translatedFormat('d F Y') }}</p>
    <form method="POST" action="{{ route('tenant.clients.plan-action-morale.generer', $client) }}">
        @csrf
        <p class="wd-reco-question">Contexte complémentaire</p>
        <textarea name="contexte" class="wd-reco-textarea" placeholder="Facultatif mais recommandé"></textarea>
        <div class="wd-reco-actions">
            <button type="submit" class="wd-reco-submit">Plan d'action</button>
        </div>
    </form>
</div>
</div>
...
@if($planAction && $planAction->status === 'completed')
    ...
    <div class="wd-reco-result">
        <div class="wd-reco-result-head">
            ...
            <button type="button" id="wd-plan-pdf-btn" class="wd-reco-submit" data-pdf-url="{{ route('tenant.clients.plan-action-morale.pdf', $client) }}" data-lieu-defaut="{{ $cabinet?->ville }}">
                Télécharger en PDF
            </button>
        </div>
        <div id="wd-plan-editor" class="wd-reco-editor">{!! $htmlContenu !!}</div>
        <form method="POST" action="{{ route('tenant.clients.plan-action-morale.modifier', ['client' => $client, 'analysis' => $planAction->id]) }}" class="wd-reco-editor-form">
            @csrf
            @method('PUT')
...
@elseif($planAction && $planAction->status === 'failed')
    <div class="wd-wrap">
        <div class="wd-reco-flash wd-reco-flash-error">La dernière génération a échoué : {{ $planAction->error_message }}</div>
    </div>
@endif
</x-tenant-app-layout>
